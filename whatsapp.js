...
// import { updateWhatsappStatus } from './utils/api-request.util.js'
...
const createSession = async (sessionId, isLegacy = false, res = null) => {
    ...
    wa.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            // await updateWhatsappStatus(sessionId, true)
        ...
        if (connection === 'close') {
            if (statusCode === DisconnectReason.loggedOut || !shouldReconnect(sessionId)) {
                ...
                // update status CS's phone ---> whatsappConnected : false                

                // await updateWhatsappStatus(sessionId, false)
        ...
        if (update.qr) {
            if (res && !res.headersSent) {
                try {
                    ...
                    res.end("<!DOCTYPE html/><html><head><title>node-qrcode</title></head><body><img src='" + qr + "'/></body></html>")
                    return
                } catch {
